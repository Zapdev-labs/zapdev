# Agent setup for `zapdev`

This is a deepsec scanning workspace. Project `zapdev` was just registered
(target: `..`). Setup is incomplete — `data/zapdev/INFO.md`
...
2. **Fill in `data/zapdev/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id zapdev
pnpm deepsec process --project-id zapdev
```
